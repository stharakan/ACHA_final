...
\\
	...
	$$
	\\
	We take a first row and a first column(without first element) as it was shown in the above matrix. We put a first row and zero and first column without first element(it was flipped). So the vector's length is $2n$.
	\\
	$$
	C_{2n} =\begin{bmatrix}
	...
	It is important to note that matrix contains toeplitz submatrix on diagonal position. It is possible to generate $C_{2n}$ matrix and then toeplitz $T_{n}$.
	\\
	There is \textbf{Hadamard} matrix is a square matrix whose entries are either $+1$ or $-1$ and whose rows are mutually orthogonal.
