...
switch type
    case 'RPMS'
        ...
        OMEGA(OMEGA>.5) = 1;
        OMEGA(OMEGA<=.5) = -1;
        A = @(z) A_c(z, OMEGA,K, phase, 'RPMS');
        At = @(z) At_c(z, N, OMEGA,K, phase, 'RPMS');
    case 'SUB'
        P = 1:N;
        P=P';
        %q=1:N;
        ...
        OMEGA = q(1:K)';
        OMEGA = sort(OMEGA);
        A = @(z) A_c(z, OMEGA,K, phase, 'SUB');
        At = @(z) At_c(z, N, OMEGA,K, phase, 'SUB');
end
...
xp = tvqc_logbarrier(x0, A, At, y, eps, 1e-1, 5, 1e-8, 600);
toc
